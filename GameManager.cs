...
public class GameManager : MonoBehaviour {
	...
	public GameObject[,] Pieces = new GameObject[_boardSize,_boardSize];
	...
	void CreatePiece(string _pieceName, int _posX, int _posY, int _playerTag)
	{
		...
		Pieces[_posX,_posY] = _PieceToCreate;
	}
	...
	public void MovePiece(Vector2 _coordToMove)
	{
		...
		if(validMovementBool)
		{
			...
			Pieces[(int)_coordToMove.x, (int)_coordToMove.y] = Pieces[(int)_coordPiece.x, (int)_coordPiece.y];
			Pieces[(int)_coordPiece.x , (int)_coordPiece.y ] = null;
	...
	public void EatPiece(GameObject _PieceToEat)
	{
		...
		if(TestMovement (SelectedPiece, _coordToEat) && (SelectedPiece.name != "Pawn" ||  _deltaX != 0))
		{
			...
			Pieces[(int)_coordToEat.x, (int)_coordToEat.y] = null; 
